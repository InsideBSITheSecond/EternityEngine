...
namespace eve {
	EveModel::EveModel(EveDevice &device, const std::vector<Vertex> &vertices) : eveDevice {device} {
	...
	void EveModel::createVertexBuffers(const std::vector<Vertex> &vertices) {
		...
		void *data;
		vkMapMemory(eveDevice.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(eveDevice.device(), vertexBufferMemory);
	}
